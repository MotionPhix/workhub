...
This document is intended as an **instruction manual for LLM agents** (e.g., MCP servers).  
When generating or editing code for this project, **you must always respect the following rules**:

1. **Follow the Project Structure**  
   - Place code in the correct module (Admin, Manager, Employee).  
   - Respect the separation of concerns (frontend, backend, routes).  

2. **Respect Folder and Naming Conventions**  
   - All folders under `resources` must be lowercase.  
   - Vue component filenames must use **PascalCase**.  
   - Variables and functions use **camelCase**.  
   - Routes and URLs use **kebab-case**.  

3. **Use the Correct UI Libraries**  
   - Modals → [InertiaUI Modal](https://github.com/inertiajs/inertia-modal)  
   - Toasts → `vue-sonner`  
   - Charts → `vue3-apexcharts`  
   - Styling → Tailwind CSS + shadcn-vue  
   - Icons → `lucide-vue-next` icons

4. **Use TypeScript and Vue 3 Composition API**  
   - Always start components with `<script setup lang="ts">`.  
   - Templates should follow script blocks.  

5. **Keep Code Maintainable**  
   - Extract reusable logic into composables.  
   - Keep components small and focused.  
   - Ensure UI is responsive, modern, dark-mode-ready, and accessible.  

⚠️ If generating code that violates these rules, you must **refuse and explain** why, instead of producing invalid output.

---

# 🧠 AI Integration Guidelines for Laravel + Inertia.js + Vue 3 + TypeScript

These guidelines define how AI agents (LLMs) should interact with and generate code for this project.  
They ensure **consistency, maintainability, and alignment** with the project's architecture and tooling.

---

## 📦 Project Structure and Modular Design

This project follows a **modular, separation-of-concerns** architecture, broken down into three main sections:

- **Admin**  
- **Manager**  
- **Employee**

Each module is designed to be functionally independent, yet capable of integrating seamlessly with the others when required.  
This design pattern enforces clarity, scalability, and maintainability.
...
- **Manager pages** → `resources/js/pages/manager`  
- **Employee pages** → All folders except `admin`, `manager`, and `auth` in `resources/js/pages/*`  

This ensures that UI responsibilities are clearly divided between roles.
...
- **Employee controllers** → All other controllers under `app/Http/Controllers`, excluding Admin and Manager folders.  

This logical separation keeps business logic **role-specific** and easier to manage.
...
- **Employee routes (default web interface)** → `routes/web.php`  

These route files should only include route definitions relevant to the corresponding module.
...
- Ensure the UI is modern and clean, using:

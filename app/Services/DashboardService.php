...
{
    /**
     * Calculate total hours for a user from datetime fields
     */
    private function calculateTotalHours(int $userId): float
    {
        return WorkEntry::where('user_id', $userId)
            ->whereNotNull('start_date_time')
            ->whereNotNull('end_date_time')
            ->get()
            ->sum(function ($entry) {
                return $entry->hours_worked; // Uses the computed property
            });
    }

    /**
     * Calculate total hours from a collection of work entries
     */
    private function calculateHoursFromCollection($entries): float
    {
        if ($entries instanceof \Illuminate\Database\Query\Builder || $entries instanceof \Illuminate\Database\Eloquent\Builder) {
            $entries = $entries->get();
        }

        return $entries->sum(function ($entry) {
            return $entry->hours_worked ?? 0; // Uses the computed property
        });
    }

    /**
     * Calculate average hours from a collection of work entries
     */
    private function calculateAverageHoursFromCollection($entries): float
    {
        if ($entries instanceof \Illuminate\Database\Query\Builder || $entries instanceof \Illuminate\Database\Eloquent\Builder) {
            $entries = $entries->get();
        }

        return $entries->avg(function ($entry) {
            return $entry->hours_worked ?? 0; // Uses the computed property
        }) ?: 0;
    }
    ...
    public function getBaseStats(int $userId): array
    {
        ...
        return [
            'base_metrics' => [
                ...
                'total_hours_trend' => $this->calculateMetricTrend($userId, 'hours'),
                'daily_average_trend' => $this->calculateMetricTrend($userId, 'daily_average'),
                'completed_tasks_trend' => $this->calculateMetricTrend($userId, 'tasks'),
                'completion_rate_trend' => $this->calculateMetricTrend($userId, 'completion_rate'),
            ],
            'user_targets' => $this->getUserTargets($userId),
    ...
    private function getRecentActivity(int $userId): array
    {
        ...
        return [
            'recent_entries' => WorkEntry::where('user_id', $userId)
                ->where('start_date_time', '>=', $thirtyDaysAgo)
                ...
                ->map(function ($entry) {
                    return [
                        'id' => $entry->uuid,
                        'date' => $entry->start_date_time?->format('Y-m-d') ?? 'N/A',
            ...
            'activity_summary' => [
                ...
                'total_hours_this_month' => $this->calculateHoursFromCollection(
                    WorkEntry::where('user_id', $userId)
                        ->whereMonth('start_date_time', Carbon::now()->month)
                ),
    ...
    private function calculateCompletionRate(int $userId): float
    {
        ...
        $entries = WorkEntry::where('user_id', $userId)
            ->whereMonth('start_date_time', $thisMonth)
            ->get();
    ...
    private function calculateDailyAverage(int $userId): float
    {
        ...
        $workDays = WorkEntry::where('user_id', $userId)
            ->whereMonth('start_date_time', $thisMonth)
            ->distinct('start_date_time')
            ->count();

        $totalHours = $this->calculateHoursFromCollection(
            WorkEntry::where('user_id', $userId)
                ->whereMonth('start_date_time', $thisMonth)
        );
    ...
    {
        return round($this->calculateAverageHoursFromCollection(
            WorkEntry::where('user_id', $userId)
                ->whereMonth('start_date_time', Carbon::now()->month)
        ), 2);
    }
    ...
    /**
     * Get user targets based on their department's settings
     */
    private function getUserTargets(int $userId): array
    {
        $user = User::with('department')->find($userId);

        if (! $user || ! $user->department) {
            // Fallback to system defaults if user or department not found
            return [
        ...
        return [
            'daily_hours_target' => (float) $department->working_hours_per_day,
            'daily_tasks_target' => (int) $department->daily_tasks_target,
            'quality_target' => (float) $department->quality_target_percentage,
        ];
    }

    /**
     * Calculate trend data for metrics compared to previous period
     */
    private function calculateMetricTrend(int $userId, string $metricType): array
    {
        $currentPeriod = $this->getCurrentPeriodMetric($userId, $metricType);
        $previousPeriod = $this->getPreviousPeriodMetric($userId, $metricType);

        // Handle division by zero and edge cases
        if ($previousPeriod == 0 || $previousPeriod === null) {
            // If previous period is 0 but current has data, it's an increase
            if ($currentPeriod > 0) {
                return [
                    'percentage' => 100,
                    'direction' => 'up',
        ...
        $percentage = round((($currentPeriod - $previousPeriod) / $previousPeriod) * 100, 1);
        ...
        return [
            'percentage' => abs($percentage), // Always return positive percentage
            'direction' => $direction,
        ];
    }

    /**
     * Get current period metric value
     */
    private function getCurrentPeriodMetric(int $userId, string $metricType): float
    {
        $currentMonth = Carbon::now()->startOfMonth();
        $entries = WorkEntry::where('user_id', $userId)
        ...
        return match ($metricType) {
            'hours' => (float) $this->calculateHoursFromCollection($entries),
            'daily_average' => (float) $this->calculateAverageHours($userId),
            'tasks' => (float) $entries->where('status', 'completed')->count(),
            'completion_rate' => $entries->count() > 0 ?
              ($entries->where('status', 'completed')->count() / $entries->count()) * 100 : 0.0,
            default => 0.0
        };
    }

    /**
     * Get previous period metric value
     */
    private function getPreviousPeriodMetric(int $userId, string $metricType): float
    {
        $previousMonth = Carbon::now()->subMonth()->startOfMonth();
        $entries = WorkEntry::where('user_id', $userId)
            ->whereMonth('start_date_time', $previousMonth)
            ->get();

        return match ($metricType) {
            'hours' => (float) $this->calculateHoursFromCollection($entries),
            'daily_average' => $entries->count() > 0 ?
              round($this->calculateHoursFromCollection($entries) / max($entries->unique('start_date_time')->count(), 1), 2) : 0.0,
            'tasks' => (float) $entries->where('status', 'completed')->count(),
            'completion_rate' => $entries->count() > 0 ?
              ($entries->where('status', 'completed')->count() / $entries->count()) * 100 : 0.0,
            default => 0.0

...
class WorkEntryController extends Controller
{
    private array $rules = [
        ...
        'mood' => ['nullable', 'in:high,focused,moderate,low,distracted'], // Simplified energy levels
        // Removed: productivity_rating, tools_used, weather_condition - not industry standard
    ...
    public function form(Request $request, ?WorkEntry $entry = null)
    {
        ...
        $query = \App\Models\Project::with(['department', 'manager'])
            ->where('status', '!=', 'cancelled');

        // If user has department restriction and is not admin
        $user = Auth::user();
        if ($user->department_uuid && ! $user->hasRole('admin')) {
            $query->where(function ($q) use ($user) {
                $q->where('department_uuid', $user->department_uuid)
                    ->orWhere('is_shared', true);
            });
        }

        $projects = $query->orderBy('name')->get(['uuid', 'name', 'status', 'due_date']);
    ...
    public function store(Request $request)
    {
        try {
            ...
            Log::info('Work entry store request', [
                'data' => $request->all(),
            ...
            Log::error('Validation failed for work entry', [
                'errors' => $e->errors(),
                'data' => $request->all(),
            ]);
            ...
            Log::error('Work entry creation failed', [
                'error' => $e->getMessage(),
                'data' => $request->all(),
            ]);
    ...
    public function update(Request $request, WorkEntry $entry)
    {
        try {
            ...
            // Debug: Log the incoming request data
            Log::info('Work entry update request', [
                'data' => $request->all(),
                'work_type' => $request->get('work_type'),
                'entry_id' => $entry->id,
            ]);
            ...
            Log::error('Validation failed for work entry update', [
                'errors' => $e->errors(),
                'data' => $request->all(),
                'entry_id' => $entry->id,
            ]);
            ...
            Log::error('Work entry update failed', [
                'error' => $e->getMessage(),
                'data' => $request->all(),
                'entry_id' => $entry->id,
            ]);
